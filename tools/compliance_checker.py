"""
compliance_checker.py — Automated brand compliance checker (any project)
Usage: python tools/compliance_checker.py --project ecoWorld --phase 1
       python tools/compliance_checker.py --project ecoWorld --phase all
"""
...
def write_report(results: dict, project: str, phase_num: int, brand: dict = None):
    phase_dir = PROJECT_ROOT / project / f"phase_{phase_num}"
    report_path = phase_dir / "compliance_report_auto.md"

    lines = [
        f"# Auto Compliance Report — Phase {phase_num}",
        f"## {brand.get('brand_name', project)} · Auto-generated by compliance_checker.py",
        f"",
        f"**Date:** {results['date']}",
